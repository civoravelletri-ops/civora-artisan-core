...
// Queste variabili globali sono usate per assicurarsi che Firebase sia inizializzato una sola volta,
// anche se la funzione Vercel viene "richiamata a caldo" più volte.
...
export default async function handler(req, res) {
    ...
    // --- INIZIALIZZAZIONE LAZY DI FIREBASE ADMIN SDK ---
    // Questo blocco assicura che Firebase sia inizializzato solo la prima volta che la funzione viene chiamata
    // (o dopo un "cold start" del server), e che il database 'firestoreDb' sia sempre disponibile.
    if (!initialized) {
        try {
            if (!admin.apps.length) { // Controlla se l'app Firebase Admin è già stata inizializzata
                const serviceAccount = process.env.FIREBASE_SERVICE_ACCOUNT_KEY;
                if (!serviceAccount) {
                    console.error("🔥 ERRORE: Manca FIREBASE_SERVICE_ACCOUNT_KEY.");
                    return res.status(500).json({ error: 'Firebase Service Account Key missing.' });
                }
                admin.initializeApp({
            ...
            if (admin.apps.length > 0) {
                firestoreDb = admin.firestore(); // Assegna l'istanza di Firestore alla variabile globale
                initialized = true; // Imposta il flag a 'true' per indicare che l'inizializzazione è avvenuta con successo
            } else {
                console.error("🔥 Errore: Firebase Admin App non inizializzata dopo il tentativo.");
                return res.status(500).json({ error: 'Firebase Admin App non inizializzata.' });
            }
        } catch (error) {
            console.error("🔥 Errore Init Firebase nell'handler:", error);
            return res.status(500).json({ error: `DB non operativo. Errore di inizializzazione: ${error.message}` });
        }
    }

    // Ora 'firestoreDb' dovrebbe essere sempre inizializzato.
    // Assegniamo 'db' come alias locale per comodità, così il resto del codice non deve cambiare molto.
    const db = firestoreDb; 
    
    if (!db) { // Ulteriore controllo di sicurezza, se per qualche ragione 'firestoreDb' è ancora nullo.
        console.error("DB non operativo. Firebase Admin SDK non è stato inizializzato con successo all'uscita del blocco.");
    ...
    try {
        ...
        // CONFERMATO: Il tuo modello AI perfetto per il 2026.
        ...
        // =====================================================================================
            ...
            try {
                ...
                console.log("Chiamando MacroDroid Webhook per invio SMS:", finalMacroDroidUrl);

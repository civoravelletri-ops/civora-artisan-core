                    ...
                    // --- INIZIO: LOGICA CONTATORE GLOBALE FIRESTORE ADMIN (con i tuoi nomi personalizzati e la tua variabile d'ambiente) ---
                                if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) { // Ora usa il tuo nome di variabile
                                    try {
                                        ...
                                        // 1. Aggiorna il contatore globale di base (rimane attivo come prima)
                                        await globalStatsRef.update({
                                            total_generated_images_ai: admin.firestore.FieldValue.increment(1)
                                        });

                                        // 2. NUOVO: Aggiorna la statistica mensile specifica per l'attività corrente
                                        if (vendorId) {
                                            try {
                                                const now = new Date();

                                                // Calcoliamo anno e mese basandoci sul fuso orario italiano (Europe/Rome)
                                                const options = { timeZone: 'Europe/Rome', year: 'numeric', month: '2-digit' };
                                                const formatter = new Intl.DateTimeFormat('it-IT', options);
                                                const parts = formatter.formatToParts(now);

                                                const anno = parts.find(p => p.type === 'year').value; // Es: "2026"
                                                const mese = parts.find(p => p.type === 'month').value; // Es: "08"

                                                // Creiamo l'ID unico del documento combinando ID Negozio + Anno + Mese
                                                const docId = `${vendorId}_${anno}_${mese}`;

                                                // Puntiamo al percorso: civora_analytics ➔ ai_gen ➔ activities ➔ idattivita_anno_mese
                                                const activityRef = db.collection('civora_analytics')
                                                                      .doc('ai_gen')
                                                                      .collection('activities')
                                                                      .doc(docId);

                                                // Usiamo il comando set con merge: true. Se il documento mensile per quel negozio
                                                // non esiste ancora, viene creato da zero; se esiste già, incrementa solo il contatore.
                                                ...
                                                console.log(`Statistiche salvate per negozio ${vendorId} - Periodo ${anno}/${mese}`);

                                            } catch (statsError) {
                                                console.error("Errore durante il salvataggio delle statistiche dettagliate:", statsError);
                                                // Non blocchiamo il flusso principale se fallisce solo il tracciamento statistico
                                            }
                                        }

...
if (!admin.apps.length) {
    ...
    if (firebaseServiceAccountKey) {
        try {
            ...
            try {
                // Tenta di decodificare Base64 se JSON diretto fallisce
...
// ==================================================================
// 2. FUNZIONI DI SUPPORTO AI (Groq API e Traduzioni)
// ==================================================================
...
// ==================================================================
// 3. LOGICA SPECIFICA PER AZIONE: teach_ai_collaborator (Agrigarden Dashboard)
// ==================================================================
async function handleTeachAICollaborator(req, res, groqApiKey) {
    ...
    const newConsolidatedMemory_it = await callGroqAPI(summarySystemPrompt, summaryUserPrompt, groqApiKey, 0.3);
...
// ==================================================================
// 4. LOGICA SPECIFICA PER AZIONE: botanico (Agrigarden Frontend)
// ==================================================================
async function handleBotanicoClient(req, res, groqApiKey) {
    const { contesto } = req.body;
    const vendorId = contesto.vendorId; // Assicurati che il vendorId arrivi nel contesto

    // Carica la memoria strutturata e il riassunto dal database per il Botanico
    ...
    if (vendorId) {
        ...
        if (aiMemoryDoc.exists) {
            ...
            // Prende l'istruzione generale nella lingua del cliente, fallback all'italiano
            generalInstructions = data.instructions_i18n?.[contesto.lang] || data.instructions || "Nessuna istruzione personalizzata.";
        }
    }
    ...
    CONTESTO PERSONALIZZATO DAL NEGOZIANTE (ESTRATTO DALLA MEMORIA STRUTTURATA):
    ${JSON.stringify(structuredMemory, null, 2)}
    ISTRUZIONI GENERALI DAL NEGOZIANTE:
    "${generalInstructions}"
    ...
    try {
        const aiResponse = await callGroqAPI(systemPrompt, userPromptText, groqApiKey, 0.5, { type: "json_object" });
...
// ==================================================================
// 5. LOGICA SPECIFICA PER AZIONE: carrelli (Vecchia logica Personal Shopper)
// ==================================================================
...
// ==================================================================
// 6. MAIN HANDLER (Entry point della funzione Vercel)
// ==================================================================
export default async function handler(req, res) {
    // A. REGOLE DI SICUREZZA (CORS)
    ...
    // B. RISPOSTA IMMEDIATA AL BROWSER PER IL CONTROLLO OPTIONS
    if (req.method === 'OPTIONS') {
        res.status(200).end();
        return;
    }

    // C. CONTROLLO METODO E DATABASE
    if (req.method !== 'POST') return res.status(405).json({ error: 'Method Not Allowed' });
    if (!db) return res.status(500).json({ error: 'Database Firebase non connesso' });

    // D. ESTRAZIONE DATI SICURA E CHIAVE API
    ...
    try {
        switch (action) {
            ...
            case 'carrelli': // Vecchia azione per i carrelli del Personal Shopper
                return await handlePersonalShopperCarrelli(req, res, GROQ_API_KEY);
            // AGGIUNGI QUI ALTRE AZIONI PER ALTRI SETTORI SE NE HAI
            // case 'altra_azione':
            //     return await handleAltraAzione(req, res, GROQ_API_KEY);
            default:
                return res.status(400).json({ error: 'Azione sconosciuta o non supportata.' });
        }
    } catch (error) {
        console.error("❌ ERRORE GENERALE HANDLER:", error);
        return res.status(500).json({ error: error.message });
    }
}
